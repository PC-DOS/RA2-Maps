...
scpeqDrPicsellDois_Skill_UpperLayerNarrator_HPProtect = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
        ...
        if player:hasSkill(self:objectName()) then
            ...
            -- This will make the game very noisy, ignored
            ...
            if event == sgs.Dying or event == sgs.Death or event == sgs.BuryVictim or event == sgs.BeforeGameOverJudge then
                if not player:isAlive() and room:askForSkillInvoke(player, "scpeqDrPicsellDois_Skill_UpperLayerNarrator_HPProtect_Reviving", data) then
                    room:revivePlayer(player)
                    --player:setAlive(true)
                    room:drawCards(player, 5, self:objectName())
                    dthDeath = data:toDeath()
                    if dthDeath then
                        dthDeath.who = nil
                    end
                    data:setValue(dthDeath)
                end
            end
        end
    end,
...
scpeqDrPicsellDois_Skill_UpperLayerNarrator_HPProtect_Hidden1 = sgs.CreateTriggerSkill{
    ...
    on_trigger = function(self, event, player, data)
        ...
        for _,plrSkillOwner in sgs.list(room:findPlayersBySkillName(self:objectName())) do
...
sgs.LoadTranslationTable{
    ...
    [":scpeqDrPicsellDois_Skill_UpperLayerNarrator_HPProtect"] = "锁定技。你的体力值和体力上限永远不小于5。当你受到伤害、失去体力或体力值变更时，你将体力值恢复到体力上限。当你的体力上限减少至5以下时，你将体力上限变为5。当你死亡时，你可将体力值调整到体力上限并摸5张牌。当你失去记载在卡面上的技能时，防止之。当你获得不在卡面上记载的技能时，你可选择获得之或丢弃之。当你被翻面时，取消之。当你的装备区被废除时，防止之。当你成为牌的目标时，你可取消之。",
